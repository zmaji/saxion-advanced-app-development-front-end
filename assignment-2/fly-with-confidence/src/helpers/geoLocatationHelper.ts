...
export const getCurrentCity = async (latitude: number, longitude: number) => {
  try {
    const response = await axios.get(
        ...
        ?key=${OPEN_CAGE_DATA_API_KEY}&q=${latitude}%2C+${longitude}&pretty=1&no_annotations=1`,
    );
...
export const getNearbyAirports = async (latitude: number, longitude: number, distance: number) => {
  try {
    const response = await axios.get(`https://airlabs.co/api/v9/nearby?lat=${latitude}
    &lng=${longitude}
    &distance=${distance}
